...
logging.basicConfig(filename="dwsongs.log", level=logging.INFO,
        ...
        try:
            if thing.json()['error']['message'] == "Quota limit exceeded":
                sendMessage(chat_id, translate(
                    languag[chat_id], "Please send the link again :("))
                return
        except KeyError:
            pass
        try:
            if thing.json()['error']:
                sendMessage(chat_id, translate(
                    languag[chat_id], "No result has been found :("))
                return
        except KeyError:
            pass
    return thing
...
def translate(language, sms):
    try:
        ...
        api = "https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20181114T193428Z.ec0fb3fb93e116c0.24b2ccfe2d150324e23a5571760e9a827d953003&text=%s&lang=en-%s" % (
            sms, language)
    ...
    try:
        ...
        if time <= 4:
            ...
            if time <= 4 and date[chat_id]['msg'] > 4:
                ...
                sendMessage(chat_id, translate(
                    languag[chat_id], "It is appearing you are trying to flood, you have to wait more that four second to send another message.\n" + str(date[chat_id]['tries']) + " possibilites :)"))
                if date[chat_id]['tries'] == 0:
                    write_db("INSERT INTO BANNED(banned) values('%d')" %
                             chat_id)
                    del date[chat_id]
                    sendMessage(chat_id, translate(
                        languag[chat_id], "You are banned :)"))
    except KeyError:
        try:
...
def sendAudio(chat_id, audio, link=None, image=None, youtube=False):
    ...
    try:
        ...
        if os.path.isfile(audio):
            ...
            try:
                request = requests.post(
                    url, params=data, files=file, timeout=20)
            except:
                request = requests.post(
                    url, params=data, files=file, timeout=20)
            if request.status_code != 200:
                sendMessage(chat_id, translate(
                    languag[chat_id], "The song " + tag['artist'][0] + " - " + tag['title'][0] + " is too big to be sent :("))
            else:
                if youtube == False:
        ...
        sendMessage(chat_id, translate(
            languag[chat_id], "Sorry the track doesn't seem readable on Deezer :("))
    except:
        sendMessage(chat_id, translate(
            languag[chat_id], "Sorry for some reason I can't send the track :("))
...
def Link(link, chat_id, quality, msg):
    ...
    try:
            ...
            if "track/" in link:
                ...
                try:
                    ...
                    if not "The access token expired" in str(a):
                        sendMessage(chat_id, translate(
                            languag[chat_id], "Invalid link ;)"), reply_to_message_id=msg['message_id'])
                        delete(chat_id)
                        return
                    spo = spotipy.Spotify(auth=generate_token())
                    url = spo.track(link)
                ...
                for a in tracks['tracks']['items']:
                    count += a['duration_ms']
                    c.execute("SELECT query FROM DWSONGS WHERE id = '%s' and quality = '%s'" % (
                        a['external_urls']['spotify'], quality.split("MP3_")[-1]))
                    links2.append(a['external_urls']['spotify'])
                    if c.fetchone() != None:
                        links1.append(a['external_urls']['spotify'])
                if (count / 1000) > 40000:
                    sendMessage(chat_id, translate(
                        languag[chat_id], "If you do this again I will come to your home and I will ddos your ass :)"))
                    delete(chat_id)
                    return
                ...
                for a in tracks['tracks']['items']:
                    try:
                        track(a['track']['external_urls']
                              ['spotify'], chat_id, quality)
                    except KeyError:
                        try:
                            sendMessage(
                                chat_id, a['track']['name'] + " Not found :(")
                        except KeyError:
                ...
                sendMessage(chat_id, translate(
                    languag[chat_id], "Sorry :( The bot doesn't support this link"))
        elif "deezer" in link:
            if "track/" in link:
                if "?" in link:
                    link, a = link.split("?")
                ...
                if image1 == None:
                    ...
                    image1 = BeautifulSoup(image1, "html.parser").find(
                        "img", class_="img_main").get("src").replace("120x120", "1000x1000")
                ...
                if "?" in link:
                    link, a = link.split("?")
                try:
                    url = request("https://api.deezer.com/album/" +
                                  link.split("/")[-1], chat_id, True).json()
                except AttributeError:
                    delete(chat_id)
                    return
                ...
                if image1 == None:
                    ...
                    image1 = request(URL).text
                    image1 = BeautifulSoup(image1, "html.parser").find(
                        "img", class_="img_main").get("src").replace("200x200", "1000x1000")
                ima = request(image1).content
                if len(ima) == 13:
                    image1 = "https://e-cdns-images.dzcdn.net/images/cover/1000x1000-000000-80-0-0.jpg"
                ...
                if "?" in link:
                    link, a = link.split("?")
                try:
                    url = request("https://api.deezer.com/playlist/" +
                                  link.split("/")[-1], chat_id, True).json()
                except AttributeError:
                    delete(chat_id)
                    return
                ...
                sendPhoto(chat_id, url['picture_xl'], caption="Artist:" + url['name'] + "\nAlbum numbers:" + str(url['nb_album']) + "\nFans on Deezer:" + str(url['nb_fan']),
                    ...
                    inline_keyboard=[
                        [InlineKeyboardButton(text="TOP 30", callback_data=link + "/top?limit=30"),
                         InlineKeyboardButton(text="ALBUMS", callback_data=link + "/albums")],
                        [InlineKeyboardButton(
                            text="RADIO", callback_data=link + "/radio")]
                    ]
                ))
            else:
                sendMessage(chat_id, translate(
        ...
        sendMessage(chat_id, translate(
            languag[chat_id], "Album didn't find on Deezer :("))
        sendMessage(chat_id, translate(
            languag[chat_id], "Try to search it throught inline mode or search the link on Deezer"))
    except Exception as a:
        logging.warning(a)
        logging.info(link)
        sendMessage(chat_id, translate(
            languag[chat_id], "OPS :( Something went wrong please contact @An0nimia to explain the issue, if this happens again, and be sure to using @DeezloaderRMX_bot"))
    try:
        if done == 1:
            sendMessage(chat_id, translate(
                languag[chat_id], "FINISHED :)"), reply_to_message_id=msg['message_id'])
    except:
        pass
    delete(chat_id)
...
def Audio(audio, chat_id):
    ...
    try:
        ...
        sendMessage(chat_id, translate(
            languag[chat_id], "File sent is too big, please send a file lower than 20 MB"))
        return
    ...
    if audio['status']['msg'] != "Success":
        sendMessage(chat_id, translate(
            languag[chat_id], "Sorry cannot detect the song from audio :(, retry..."))
    else:
        ...
        try:
            url = request("https://api.deezer.com/search/track/?q=" + track.replace(
                "#", "") + " + " + artist.replace("#", ""), chat_id, True).json()
        except AttributeError:
            return
        try:
            ...
            try:
                id = "https://open.spotify.com/track/" + \
                    audio['metadata']['music'][0]['external_metadata']['spotify']['track']['id']
                try:
                    url = spo.track(id)
                except:
                    spo = spotipy.Spotify(auth=generate_token())
            ...
            try:
                id = "https://api.deezer.com/track/" + \
                    str(audio['metadata']['music'][0]
                        ['external_metadata']['deezer']['track']['id'])
                try:
        ...
        try:
            sendPhoto(chat_id, image, caption=track + " - " + artist,
                      reply_markup=InlineKeyboardMarkup(
                          inline_keyboard=[
                              [InlineKeyboardButton(text="Download", callback_data=id), InlineKeyboardButton(
                                  text="Info", callback_data=album)]
                          ]
                      ))
        except:
    ...
    if "artist" in query_data:
        ...
        if "album" in query_data:
            try:
                ...
                return
            for a in url['data']:
                array.append([InlineKeyboardButton(text=a['title'] + " - " +
                                                   a['release_date'].replace("-", "/"), callback_data=a['link'])])
            array.append([InlineKeyboardButton(
                text="BACK 🔙", callback_data=query_data.split("/")[-2] + "/" + "artist")])
            ...
            if ans == "2":
                if users[from_id] == 3:
                    bot.answerCallbackQuery(query_id, translate(
                        languag[from_id], "Wait the end and repeat the step, did you think you could download how much songs you wanted? ;)"), show_alert=True)
                    return
                else:
                    users[from_id] += 1
            bot.answerCallbackQuery(query_id, translate(
            ...
            try:
                url = request("https://api.deezer.com/artist/" + query_data.split("/")
                              [-4] + "/" + query_data.split("/")[-1], from_id, True).json()
            except AttributeError:
                return
            for a in url['data']:
                Link("https://www.deezer.com/track/" +
                     str(a['id']), from_id, qualit[from_id], msg['message'])
                if ans == "2":
                    users[from_id] += 1
            if ans == "2":
                users[from_id] -= 1
            ...
            try:
                ...
                return
            for a in url['data']:
                array.append([InlineKeyboardButton(text=a['artist']['name'] + " - " +
                                                   a['title'], callback_data="https://www.deezer.com/track/" + str(a['id']))])
            array.append([InlineKeyboardButton(text="GET ALL ⬇️", callback_data=query_data.split(
                "/")[-2] + "/" + "artist/down/" + query_data.split("/")[-1])])
            array.append([InlineKeyboardButton(
                text="BACK 🔙", callback_data=query_data.split("/")[-2] + "/" + "artist")])
            bot.editMessageReplyMarkup(((from_id, message_id)),
                                       reply_markup=InlineKeyboardMarkup(
            ...
            try:
                url = request("https://api.deezer.com/artist/" +
        ...
        if tags[0] == "Infos with too many bytes":
            bot.answerCallbackQuery(
                query_id, translate(languag[from_id], query_data))
        elif len(tags) == 4:
            bot.answerCallbackQuery(
                query_id, text="Album: " + tags[0] + "\nDate: " + tags[1] + "\nLabel: " + tags[2] + "\nGenre: " + tags[3], show_alert=True)
        else:
            if ans == "2":
                if users[from_id] == 3:
                    bot.answerCallbackQuery(query_id, translate(
...
def search(msg):
    ...
    if "" == query_string:
        ...
        result = [InlineQueryResultArticle(id=a['link'], title=a['title'], description=a['artist']['name'], thumb_url=a['album']
                                           ['cover_big'], input_message_content=InputTextMessageContent(message_text=a['link'])) for a in search1['tracks']['data']]
        result += [InlineQueryResultArticle(id="https://www.deezer.com/album/" + str(a['id']), title=a['title'] + " (Album)", description=a['artist']['name'], thumb_url=a['cover_big'],
                                            input_message_content=InputTextMessageContent(message_text="https://www.deezer.com/album/" + str(a['id']))) for a in search1['albums']['data']]
        result += [InlineQueryResultArticle(id=a['link'], title=str(a['position']), description=a['name'], thumb_url=a['picture_big'],
                                            input_message_content=InputTextMessageContent(message_text=a['link'])) for a in search1['artists']['data']]
        result += [InlineQueryResultArticle(id=a['link'], title=a['title'], description="N° tracks:" + str(a['nb_tracks']), thumb_url=a['picture_big'],
                                            input_message_content=InputTextMessageContent(message_text=a['link'])) for a in search1['playlists']['data']]
    elif "album:" in query_string:
        search1 = request("https://api.deezer.com/search/album/?q=" +
                          query_string.split("album:")[1].replace("#", "")).json()
        try:
            if search1['error']:
                return
        except KeyError:
            pass
        result = [InlineQueryResultArticle(id=a['link'], title=a['title'], description=a['artist']['name'], thumb_url=a['cover_big'],
                                           input_message_content=InputTextMessageContent(message_text=a['link'])) for a in search1['data']]
    elif "artist:" in query_string:
        search1 = request("https://api.deezer.com/search/artist/?q=" +
                          query_string.split("artist:")[1].replace("#", "")).json()
        try:
            if search1['error']:
                return
        except KeyError:
        ...
        search1 = request("https://api.deezer.com/search?q=" +
                          query_string.replace("#", "")).json()
        try:
            if search1['error']:
                return
        except KeyError:
        ...
        result = [InlineQueryResultArticle(id=a['link'], title=a['title'], description=a['artist']['name'], thumb_url=a['album']
                                           ['cover_big'], input_message_content=InputTextMessageContent(message_text=a['link'])) for a in search1]
        for a in search1:
            try:
                ...
                continue
            result += [InlineQueryResultArticle(id="https://www.deezer.com/album/" + str(a['album']['id']), title=a['album']['title'] + " (Album)", description=a['artist']
                                                ['name'], thumb_url=a['album']['cover_big'], input_message_content=InputTextMessageContent(message_text="https://www.deezer.com/album/" + str(a['album']['id'])))]
    try:
...
def start(msg):
    ...
    if content_type == "text" and msg['text'] == "/start":
        try:
            sendPhoto(chat_id, open("example.png", "rb"), caption=translate(
                languag[chat_id], "Welcome to @DeezloaderRMX_bot\nPress '/' to get commands list"))
        except FileNotFoundError:
            pass
        sendMessage(chat_id, translate(languag[chat_id], "Press for search songs or albums or artists\nP.S. Remember you can do this digiting @ in your keyboard and select DeezloaderRMX_bot\nSend a Deezer or Spotify link to download\nSend a song o vocal message to recognize the track"),
                    reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="Search by artist", switch_inline_query_current_chat="artist:"), InlineKeyboardButton(
                    text="Search by album", switch_inline_query_current_chat="album:")],
                [InlineKeyboardButton(
        ...
        if languag[chat_id] != "en":
            languag[chat_id] = "en"
            sendMessage(chat_id, translate(
                languag[chat_id], "Now the language is english"))
        else:
            languag[chat_id] = msg['from']['language_code']
            sendMessage(chat_id, translate(
                languag[chat_id], "Now the bot will use the Telegram app language"))
        ...
        sendMessage(chat_id, translate(languag[chat_id], "Select default download quality"),
            ...
            keyboard=[
                [KeyboardButton(text="FLAC"), KeyboardButton(
                    text="MP3_320Kbps")],
                [KeyboardButton(text="MP3_256Kbps"), KeyboardButton(
                    text="MP3_128Kbps")]
            ]
        ))
        ...
        sendMessage(chat_id, translate(
            languag[chat_id], "Songs will be downloaded in " + msg['text'] + " quality"), reply_markup=ReplyKeyboardRemove())
        sendMessage(chat_id, translate(
            languag[chat_id], "Songs which cannot be downloaded in quality you have chosen will be downloaded in the best quality possible"))
    elif content_type == "voice" or content_type == "audio":
        Thread(target=Audio, args=(
            msg[content_type]['file_id'], chat_id)).start()
        ...
        try:
            ...
            if ans == "2" and users[chat_id] == 3:
                sendMessage(chat_id, translate(
                    languag[chat_id], "Wait the end and repeat the step, did you think you could download how much songs you wanted? ;)"))
            else:
                if ans == "2":
                    users[chat_id] += 1
                Thread(target=Link, args=(msg['text'].replace(
                    "'", ""), chat_id, qualit[chat_id], msg)).start()
        except KeyError:
            sendMessage(chat_id, translate(languag[chat_id], "Press"),
                        reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="Search artist", switch_inline_query_current_chat="artist:" + msg['text']),
                     InlineKeyboardButton(text="Search album", switch_inline_query_current_chat="album:" + msg['text'])],
                    [InlineKeyboardButton(
                        text="Search global", switch_inline_query_current_chat=msg['text'])]
                ]
            ))
